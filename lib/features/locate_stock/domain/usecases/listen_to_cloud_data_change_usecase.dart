import 'package:stock_management_tool/core/usecase/usecase.dart';
import 'package:stock_management_tool/features/locate_stock/domain/repositories/locate_stock_repository.dart';

class ListenToCloudDataChangeLocateStockUseCase extends UseCase {
  ListenToCloudDataChangeLocateStockUseCase(this._locateStockRepository);

  final LocateStockRepository _locateStockRepository;

  @override
  Future call({params}) {
    return _locateStockRepository.listenToCloudDataChange(
      locatedItems: params["locatedItems"],
      selectedItems: params["selectedItems"],
      onChange: params["onChange"],
    );
  }
}
